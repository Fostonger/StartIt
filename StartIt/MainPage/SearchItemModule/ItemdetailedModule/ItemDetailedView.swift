...
struct ItemDetailedView<Model>: View where Model: ItemDetailedViewModelInterface {
    @StateObject var viewModel: Model
    var body: some View {
        VStack {
            Text(viewModel.itemDetailedModel.item.name)
                    .font(.largeTitle)
                    .padding()
            if let image = viewModel.itemDetailedModel.image {
                image
            ...
            Text(viewModel.itemDetailedModel.item.description)
...
//            }
            ...
            Divider()
                .padding()
            Text(String(format: "%.2f", viewModel.itemDetailedModel.item.price))
                .padding()
